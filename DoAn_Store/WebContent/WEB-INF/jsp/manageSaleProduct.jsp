...
<body>
						...
						<tbody>
							<c:forEach var="saleProduct" items="${saleProductList}"
								...
								<tr>
									...
									<td>${fn:escapeXml(saleProduct.salePrice)}</td>
									<td>${fn:escapeXml(saleProduct.event.name)}</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
			...
			<div class="row top-10">
				<div class="col-md-6 col-md-offset-3">
					<div class="form-group">
						...
						<a href="manageSaleProduct?method=findAdd" class="btn btn-default">
							Add </a>
					</div>
				</div>
			</div>
	...
	<div class="text-center">
		<ul class="page-display pagination">
			<c:if test="${saleProductForm.sPage > 1}">
				<li><a
					href="${appPath}/employee/manageSaleProduct?method=list&sPage=${saleProductForm.sPage - 1}">Prev</a></li>
			</c:if>
			<c:forEach var="pageIndex" items="${requestScope.pageIndexes }">
				<c:choose>
					<c:when test="${saleProductForm.sPage eq pageIndex}">
						<li class="current-page active"><a href="#">${pageIndex}</a></li>
					</c:when>
					<c:otherwise>
						<li><a class="other-display-pages"
							href="${appPath}/employee/manageSaleProduct?method=list&sPage=${pageIndex}">${pageIndex}</a></li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<c:if
				test="${requestScope.pageIndexes[fn:length(requestScope.pageIndexes) - 1] > saleProductForm.sPage}">
				<li><a
					href="${appPath}/employee/manageSaleProduct?method=list&sPage=${saleProductForm.sPage + 1}">Next</a></li>
			</c:if>
		</ul>
	</div>
</body>
